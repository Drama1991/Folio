          ...
          >
            ...
            >
              ...
              >
                ...
                <p style={{ fontFamily: "var(--mono)", fontSize: 11, color: "#C9C7C0", marginTop: 6 }}>
                  {[featured.creator, featured.year, mediumLabel(featured.medium)].filter(Boolean).join(" · ")}
                </p>
                {featured.brief && (
                  <p style={{ fontSize: 12, color: "#D6D4CD", marginTop: 14, lineHeight: 1.6 }}>
      ...
      {trending.map((sec) => (
        <div key={sec.medium}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
            ...
            <Link href={`/discover/${sec.medium}`} style={{ fontFamily: "var(--mono)", fontSize: 10, color: "var(--text3)", textDecoration: "none" }}>
              全部热门 →
            </Link>
          </div>
          {sec.items.length > 0 ? (
